...
export function generateLook(person) {
  ...
  else if (role === 'tourist' && season === 'summer' && hatRoll < 0.5)
    accessories.push('straw-hat');
...
function finish(look) {
  ...
  return Object.freeze({
    ...
    accessories: Object.freeze(
      [...new Set(look.accessories)].filter((a) => ACCESSORIES.includes(a)),
    ),
...
export function namedLook(role, overrides = {}) {
  ...
  return finish({
    ...
    colors: {
      ...paletteFor(seededRandom(hashString(key)), { body, outfit: entry.outfit, accessories: [] }),
      ...entry.colors,
      ...overrides.colors,
    },
    scale: entry.scale ?? 1,

...
export function createCrowd({
  ...
  async function loadBody(entry) {
    ...
    const skinned = lod1?.isSkinnedMesh
      ? lod1
      : lod1?.children.find((child) => child.isSkinnedMesh);
    if (!skinned) {
      entry.status = 'failed';
  ...
  function createMidMesh(bodyId, source, bake) {
    ...
    const material = createKitMaterial(THREE, {
      mode: 'mid',
      uniforms,
      name: `Crowd / mid / ${bodyId}`,
    });
  ...
  function createSlot(
    bodyId,
    { personId = null, props = null, variants = {}, seatHeight = null } = {},
  ) {
    ...
    slot.hero = createHeroCast({
      ...
      prepare: ({ root }) => {
        ...
        root.traverse((node) => {
          ...
          node.material = createKitMaterial(THREE, {
            mode: 'near',
            uniforms,
            name: `Crowd / near / ${bodyId}`,
          });
  ...
  function midClip(person, body, dt) {
    ...
    if (choice.clip === walkName)
      choice.timeScale = strideTimeScale(
        person.speed,
        body.gait.speeds[walkName] ?? body.gait.walkSpeed,
      );
    return choice;
  }
  ...
  function update(dt, context = {}) {
    ...
    for (const source of sources.values())
      source.collect?.((record) => records.push(record), context);
    ...
    for (const record of records) {
      ...
      candidates.push({
        id: record.id,
        distance,
        priority: record.priority ?? (roles.has(record.id) ? 1 : 0),
      });
    }
    ...
    for (const person of people.values()) {
      ...
      person.vis =
        person.present && tier !== 'far' ? approach(person.vis, 1) : approach(person.vis, 0);
    ...
    for (const slot of slots) {
      ...
      slot.hero.update(
        slot.dedicated ? real : step,
        slot.dedicated ? { ...context.hero, paused: false } : (context.hero ?? {}),
      );
  ...
  return {
    ...
    getState() {
      return {
        ...
        tiers: stats,
        settings: {
          nearCount: settings.nearCount,
          midCount: settings.midCount,
          midEnter: settings.midEnter,
        },

...
const roles = (params.get('role') ?? 'commuter,student,elder,farmer,shopkeeper,staff,child,tourist,vendor,worker,neighbour,shopper').split(',');
...
const camera = new THREE.PerspectiveCamera(view === 'face' ? 20 : 30, innerWidth / innerHeight, 0.05, 400);
const spacing = 0.85;
const people = [];
...
const modelLoader = createModelLoader({ load: (path) => createGltfLoader().then((load) => load(path)) });
...
function tick() {
  ...
  crowd.update(dt, { camera, weather: params.get('weather') ?? 'clear', season: params.get('season') ?? 'autumn', hero: { camera } });
  if (forced === 'mid') {

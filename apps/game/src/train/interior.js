...
const THROUGH_ROLES = ['office commuter', 'school student', 'retired neighbour', 'tourist', 'market shopper'];
...
export function createCarInterior({
  ...
  function update({ dt = 0, speed = 0, power = 0, brake = 0, passengers = [], cabinOn = false }) {
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      if (!p.through) crowdRecords[i].role = passengers.find((person) => person.id === p.id)?.role ?? 'commuter';

...
Run `pnpm build:runbook` to build `artifacts/runbook-site`. The authored guide lives in `runbook/`; its 49 chapters include 2560 × 1440 game captures (chapters 38–49 use smaller captures and contact sheets from the 23 September 2026 session, plus two labelled AI-generated concept images), plain-English explanations, autoplay SVG experiments, optional code and practice sections, individual copyable skills and a downloadable skill collection. `skills-pack.json` contains the ZIP payload because Signal static hosting does not accept ZIP files directly.

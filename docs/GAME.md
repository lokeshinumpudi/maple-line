...
**Places → Watch a short drama** plays episodes of _The 17:42_, a three-part companion series staged with the director, NPC acting notes, auto drive, doors and weather. Episodes are data that agents can write, validate and play through WebMCP; see [writing episodes](drama/README.md). With the local director and a Sarvam key, each part has its own voice in English or ten Indian languages, with translated subtitles; otherwise episodes play with labelled subtitles. `pnpm voice:episode` writes the clips and an audio manifest for video renders.

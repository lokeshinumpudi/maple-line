...
**Places → Watch a short drama** plays episodes of _The 17:42_, a three-part companion series staged with the director, NPC acting notes, auto drive, doors and weather. Meera's Telugu family has lived in the valley for forty years; her Ammamma moves back to India tomorrow and wants to hear Grandpa's radio at home tonight, and Meera's train gets in two minutes after the last bus up the hill. Her classmate Arjun asks his sister Divya, the bus driver, to wait. Aonuma has a bus bay behind the kiosk end of the platform with a Blender-built cream and teal village bus (headlights, lit windows, folding front door, a few rows of seats), a shelter and a lane out to the village street. Momiji and Aonuma have readable timetable boards and station clocks whose hands follow the scene time. Episodes are data that agents can write, validate and play through WebMCP; see [writing episodes](drama/README.md). `pnpm render:episode` renders an episode to a phone-ready MP4 (16:9 or 9:16) with a poster and a line timeline for voice clips; see [episode videos](VIDEO.md). With the local director and a Sarvam key, each part has its own voice in English or ten Indian languages, with translated subtitles; otherwise episodes play with labelled subtitles. `pnpm voice:episode` writes the clips and an audio manifest for video renders. The speaking character's mouth follows the voice: from the manifest's per-line mouth curves in a render, and from an audio analyser during live play ([lip sync](VIDEO.md#lip-sync)).
...
Lamps light their surroundings at night. Each lamp, lit window and bus lamp is a light source; the ones nearest the camera borrow one of a few real lights (four point and two spot lights on High, three and one on Medium, one point light on Low), fading over about half a second when a light moves to another lamp. Every source also puts a soft pool of light on the ground and, when the ground is wet, a rippling reflection that stretches toward the camera. Halos shrink when a lamp is close to the camera or just behind someone's head. At Aonuma the platform canopies have pendant lamps; the bus headlights, cabin, tail lamps and shelter lamp are sources, and the hillside behind the bus stop has garden walls, hedges, hydrangeas, six houses with warm windows, maple and cedar stands, bamboo, wet leaves, puddles and rain splashes on the bay.

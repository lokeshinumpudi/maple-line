...
Dusk now lowers the sun to about 12° (`DUSK_SUN_OFFSET` in `main.js`), so shadows run long and the light comes in low and warm. Daylight keeps the original sun.
...
`set` accepts `weather`, `timeOfDay`, `location` (a place id or route z) and `speedKmh`. It runs through the same actions as the UI, including Places travel for location jumps, and the cut fades through black. Unfinished station duties block location jumps here, as they block Places.
